...
func (w *EntryImageWorker) processEntryImages(ctx context.Context) error {
	entries, err := w.queries.GetEntryImageNotProcessedEntries(ctx)
	if err != nil {
		return err
	}
...
func (w *EntryImageWorker) processEntry(ctx context.Context, entry admindb.Entry) error {
...
func (w *EntryImageWorker) getImageFromEntry(ctx context.Context, entry admindb.Entry) (*string, error) {
