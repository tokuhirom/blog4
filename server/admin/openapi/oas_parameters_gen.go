...
// RegenerateEntryImageParams is parameters of regenerateEntryImage operation.
type RegenerateEntryImageParams struct {
	// The path of the entry to regenerate the image for.
	Path string
}

func unpackRegenerateEntryImageParams(packed middleware.Parameters) (params RegenerateEntryImageParams) {
	{
		key := middleware.ParameterKey{
			Name: "path",
			In:   "path",
...
func decodeRegenerateEntryImageParams(args [1]string, argsEscaped bool, r *http.Request) (params RegenerateEntryImageParams, _ error) {
	// Decode path: path.
	if err := func() error {
		param := args[0]
		if argsEscaped {
